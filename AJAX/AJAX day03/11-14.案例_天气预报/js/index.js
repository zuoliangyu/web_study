...
function getWeather(code){
  myAxios({
    ...
    document.querySelector('.windDirection').innerHTML = res.data.windDirection
    document.querySelector('.today-weather .weather').innerHTML = res.data.todayWeather.weather
    ...
    document.querySelector('.week-wrap').innerHTML = res.data.dayForecast.map(item => `<li class="item">
    <div class="date-box">
      <span class="dateFormat">${item.dateFormat}</span>
      <span class="date">${item.dateFormat}</span>
    </div>
...
//1.绑定input事件，获取关键字
//2.获取展示城市列表数据
document.querySelector('.search-city').addEventListener('input',_.debounce(function(){
